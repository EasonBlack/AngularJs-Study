...
class ScheduleService {
    ...
    recodeSchedule() {
        ...
        for (let i = 0; i < this.list.length; i++) {
            ...
            if (i - 1 < 0) {
                _schedule.push({start: '0400', end: current.start, color: null, span: this.calculateSpan('0400',current.start)});
            } else if (prev.end != current.start) {
                _schedule.push({start: prev.end, end: current.start, color: null,span: this.calculateSpan(prev.end,current.start)});
            }
            ...
            if (i == this.list.length - 1) {
                _schedule.push({start: current.end, end: '2400', color: null,span: this.calculateSpan(current.end,'2400')});
            }
        }
        return _schedule;
    }
